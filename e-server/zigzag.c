 ...
 * on all cores. */
uint16_t _msg_box[ROWS*COLS+1] = { 0 };
#define MSG_BOX _msg_box

volatile uint32_t _msg_flag = 0;
#define MSG_FLAG _msg_flag

char _out_buf[ROWS*(COLS+1)*4+1] = { 0 }; /* UTF-8 worst case. */
#define sputs(S, Ignore) strncat(_out_buf, (S), sizeof(_out_buf))

/* Can't hook up to sync since e-server traps it */
int interrupt_handler(); // __attribute__ ((interrupt ("message")));
void pass_message();
void print_route();
...
void pass_message()
{
  ...
  next_msg_flag = (uint32_t *)
    e_get_global_address(E_ROW(next), E_COL(next), (void *) &MSG_FLAG);
...
inline void print_path(uint16_t path[ROWS][COLS], uint16_t row, uint16_t col)
{
  ...
  if (!prev)
    ...
    if (south == prev) sputs("|", stdout); else sputs("▲", stdout);
  else if (south == next)
    if (north == prev) sputs("|", stdout); else sputs("▼", stdout);
  else if (west == next)
    if (east == prev)  sputs("—", stdout); else sputs("◀", stdout);
  else if (east == next)
    if (west == prev)  sputs("—", stdout); else sputs("▶", stdout);
  else
    sputs("⚫", stdout);
}
